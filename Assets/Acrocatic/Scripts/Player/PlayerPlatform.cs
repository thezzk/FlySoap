...
namespace Acrocatic {
	...
	public class PlayerPlatform : MonoBehaviour {
        ...
        //edit by zzk
        public string getCurrentPlatformTag(){
            Collider2D checkCollider = Physics2D.OverlapCircle(player.groundCheck.position, player.groundRadius, player.platformLayer|player.groundLayer);
            if(checkCollider!=null){
                Transform transform=checkCollider.transform;
                GameObject checkGameobject=transform.gameObject;
                if(checkGameobject==null){
                    return "";
                }
                else{
                    return checkGameobject.tag;
                }
            }
            else 
                return "";
