...
namespace Acrocatic {
	...
	public class Player : MonoBehaviour {
        ...
        public float normalDecRate=0.2f;
        public float waterDecRate=0.4f;
        public float iceDecRate=0f;
        ...
        private float moblieInputProcess()
        {
		...
		public void Dead() {
            ...
            deathTime+=1;
            PlayerPrefs.SetInt("DeathTime",deathTime);
            if (deathTime >= 100)
            {
                AchieveTrigger.getAchieve(1);
            }
            GameObject.Instantiate(deathFx,this.transform.position,Quaternion.identity);
...
//			}
        ...
        //Process the durability edit by zzk
        public void decDurability(){
            durabilitySlider.value=currentDurability/maxDurability;
            if(hor<0.001f&&hor>-0.001f&&jumping==false&&wallJumping==false)
                return;
            string floorTag;
            floorTag=playerPlatform.getCurrentPlatformTag();
            if(floorTag.Equals("Ice")){
                currentDurability-=iceDecRate;
            }
            else if(floorTag.Equals("Water")){
                currentDurability-=waterDecRate;
            }
            else{
                currentDurability-=normalDecRate;
            }

            if(currentDurability<0.01f){
                Dead();            
            }

               
        }
        //Restore Durability edit by zzk
        public void restoreDurability(float amount){
            if(amount+currentDurability>maxDurability){
